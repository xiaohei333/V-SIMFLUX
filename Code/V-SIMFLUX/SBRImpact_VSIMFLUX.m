...
CrlbMeanStore_VSIMFLUX = zeros(params.numparams,size(azimstore,2),size(polastore,2),size(SBRstore,2));
ThetaMeanStore_VSIMFLUX = zeros(params.numparams,size(azimstore,2),size(polastore,2),size(SBRstore,2));
ThetaStdStore_VSIMFLUX = zeros(params.numparams,size(azimstore,2),size(polastore,2),size(SBRstore,2));
OutliersStore_VSIMFLUX = zeros(size(azimstore,2),size(polastore,2),size(SBRstore,2));

%% 开始计算

for ii = 1:size(SBRstore,2)
    for mm = 1:size(azimstore,2)
        for nn = 1:size(polastore,2)
            ...
            for jj = 1:params.Ncfg
                ...
                dz = 0;
                Nphotons = 4000;
                Nbackground = Nphotons/SBRstore(ii);
                dazim = azimstore(mm);
                dpola = polastore(nn);
                dg2 = 0.75;
            ...
            OutliersStore_VSIMFLUX(mm,nn,ii) = sum(outliers_VSIMFLUX,'all');
...
subplot(2,4,ii);
loglog(SBRstore,squeeze(mean(CrlbMeanStore_VSIMFLUX(ii,:,:,:),[2 3]))); hold on;
loglog(SBRstore,squeeze(mean(ThetaStdStore_VSIMFLUX(ii,:,:,:),[2 3]))); 
...
for ii = 1:params.numparams
subplot(2,4,ii);
plot(SBRstore,squeeze(mean(ThetaMeanStore_VSIMFLUX(ii,:,:,:),[2 3])));
end

figure,
for ii = 1:params.numparams
subplot(2,4,ii);
plot(SBRstore,squeeze(mean(OutliersStore_VSIMFLUX,[1 2])),'*');
end

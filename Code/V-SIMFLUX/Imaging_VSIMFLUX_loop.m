...
for jj = 1:Numloop
    ...
    if jj == 1
        ...
        for ii = 1:(params.Ncfg/SIM.L/SIM.K)
            index = (ii-1)*SIM.L*SIM.K + (1:SIM.L*SIM.K);
            thetainit_VSIMFLUX(1:3,index) = repmat(theta_VSIMFLUX(1:3,ii),[1 6]);
            % thetainit_VSIMFLUX(1:2,index) = repmat(theta_V(1:2,ii),[1 6]);
            % thetainit_VSIMFLUX(3,index) = repmat(theta_VSIMFLUX(3,ii),[1 6]);
            thetainit_VSIMFLUX(5,index) = repmat(theta_VSIMFLUX(5,ii),[1 6])./(SIM.L*SIM.K);
            thetainit_VSIMFLUX(6:8,index) = repmat(theta_VSIMFLUX(6:8,ii),[1 6]);
        end
    end
    ...
    if jj == 1
        ...
        for ii = 1:(params.Ncfg/SIM.L/SIM.K)     
            index = (ii-1)*SIM.L*SIM.K + (1:SIM.L*SIM.K);
            thetainit_VSIMFLUX(4,index) = generateNph(SIM_est,theta_VSIMFLUX(:,ii),ROIxy); % photonflux = 2.5
        end
    end
    ...
    if jj == 1
        ...
        lp1 = SearchPhasePlane(theta_VSIMFLUX(1,:)',theta_VSIMFLUX(2,:)',phase1,[85/180*pi 95/180*pi]);
        lp2 = SearchPhasePlane(theta_VSIMFLUX(1,:)',theta_VSIMFLUX(2,:)',phase2,[175/180*pi 185/180*pi]);
    end

function [thetastore,mustore,dmudthetastore,meritstore,numiters] = localization_VSIMFLUX(allspots,theta0,params,SIM,roixy0)
...
mustore = zeros(params.Mx,params.My,params.K,L,K,Ncfg);
dmudthetastore = zeros(params.Mx,params.My,params.K,numparams,L,K,Ncfg);
...
parfor jcfg = 1:Ncfg
    ...
    [mu,dmudtheta] = poissonrate_VSIMFLUX(params,SIM,theta,PupilMatrix,allzernikes,wavevector,wavevectorzimm,ROIxy);
    [merit,grad,Hessian] = likelihood_VSIMFLUX(params,spots,mu,dmudtheta,varfit);
    ...
    while ((iiter<=Nitermax) && (monitor>tollim))
       ...
       if (abs(det(matty))>2*eps)
            ...
            [mu,dmudtheta] = poissonrate_VSIMFLUX(params,SIM,thetatry,PupilMatrix,allzernikes,wavevector,wavevectorzimm,ROIxy);
            [merittry,gradtry,Hessiantry] = likelihood_VSIMFLUX(params,spots,mu,dmudtheta,varfit);
...
meritoffset = meritoffsetcalc_VSIMFLUX(allspots,params.varfit);
...
% ymin = -roisizey/2;
% ymax = roisizey/2;
xmin = -2*params.pixelsize;
xmax = 2*params.pixelsize;
ymin = -2*params.pixelsize;
ymax = 2*params.pixelsize;
